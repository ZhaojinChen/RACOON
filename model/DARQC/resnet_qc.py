...
try:
    ...
    ResNeXt50_32X4D_Weights = None
    ResNeXt101_32X8D_Weights = None
    Wide_ResNet50_2_Weights = None
    Wide_ResNet101_2_Weights = None
from torch.nn.parameter import Parameter

# based on https://github.com/pytorch/vision/blob/master/torchvision/models/resnet.py


def _load_torchvision_backbone(factory, *, pretrained=False, progress=True, weights_enum=None):
    if not pretrained:
        try:
            return factory(weights=None, progress=progress)
        except TypeError:
            try:
                return factory(pretrained=False, progress=progress)
            except TypeError:
                return factory(pretrained=False)

    if weights_enum is not None:
        try:
            return factory(weights=weights_enum.DEFAULT, progress=progress)
        except TypeError:
            return factory(weights=weights_enum.DEFAULT)

    try:
        return factory(pretrained=True, progress=progress)
    except TypeError:
...
class ResNetQC(nn.Module):
    ...
    def __init__(
        ...
        nn.init.normal_(self.addon[-1].weight, mean=0.0, std=1e-3)
        if self.addon[-1].bias is not None:
            nn.init.constant_(self.addon[-1].bias, 0.)
...
def _resnet_qc(
    ...
    **kwargs: Any
    ...
    return ResNetQC(block, dropout, layers,  **kwargs)


def resnet_qc_18(pretrained: bool = False,dropout: float = 0.2, progress: bool = True,**kwargs) -> ResNetQC:
    ...
    """
    model = _resnet_qc( BasicBlock, [2, 2, 2, 2], dropout, 
                   **kwargs)
    if pretrained:
        model_ft = _load_torchvision_backbone(
            models.resnet18,
            pretrained=True,
            progress=progress,
...
def resnet_qc_152(pretrained: bool=False, progress: bool = True, **kwargs)  -> ResNetQC:
    ...
    if pretrained:
        model_ft = _load_torchvision_backbone(
            ...
            pretrained=True,
            progress=progress,
            weights_enum=ResNet152_Weights,
        )
        model.load_from_std(model_ft)
    return model
...
def resnext_qc_50_32x4d(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNetQC:
    ...
    if pretrained:
        model_ft = _load_torchvision_backbone(
            ...
            pretrained=True,
            progress=progress,
            weights_enum=ResNeXt50_32X4D_Weights,
        )
        model.load_from_std(model_ft)
    return model
...
def resnext_qc_101_32x8d(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNetQC:
    ...
    if pretrained:
        model_ft = _load_torchvision_backbone(
            ...
            pretrained=True,
            progress=progress,
            weights_enum=ResNeXt101_32X8D_Weights,
        )
        model.load_from_std(model_ft)
    return model
...
def wide_resnet_qc_50_2(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNetQC:
    ...
    if pretrained:
        model_ft = _load_torchvision_backbone(
            ...
            pretrained=True,
            progress=progress,
            weights_enum=Wide_ResNet50_2_Weights,
        )
        model.load_from_std(model_ft)
    return model
...
def wide_resnet_qc_101_2(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNetQC:
    ...
    if pretrained:
        model_ft = _load_torchvision_backbone(
            ...
            pretrained=True,
            progress=progress,
            weights_enum=Wide_ResNet101_2_Weights,
        )
        model.load_from_std(model_ft)
    return model
