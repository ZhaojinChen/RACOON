...
class  MRIDatasetsAug(Dataset):
                ...
                with h5py.File(Fhf_path, 'r') as hf:
                    ...
                    for _, row in tqdm(file_subset.iterrows(), total=len(file_subset), desc=f"Reading {Fhf_path.split('/')[-1]}"):
                        ...
                        if self.has_transform_col:
                            affine_array = self._parse_affine_params(row['transform_params'])
                        else:
    ...
    def __getitem__(self,index):
        ...
        if self.TrainFlag:
            if self.has_total_ventricle_col and pd.notna(total_ventricle):
        ...
        if self.add_noise and self.TrainFlag:
            ...
            image, _ = add_noise(image,noise_type='R')
    ...
    def _parse_affine_params(self, raw_val):
        if raw_val is None:
            raise ValueError("Affine parameter value is NaN")
        if isinstance(raw_val, str):
            if not raw_val.strip() or pd.isna(raw_val):
                raise ValueError("Affine parameter value is NaN")
            parsed = ast.literal_eval(raw_val)
        else:
            if np.isscalar(raw_val) and pd.isna(raw_val):
                raise ValueError("Affine parameter value is NaN")
            parsed = raw_val

        affine_array = np.array(parsed, dtype=np.float32)
        if affine_array.shape != (9,):
            raise ValueError(f"The shape of params array is not correct: {affine_array.shape}")
        return affine_array
